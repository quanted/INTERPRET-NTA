...
function logspace(x1, x2, n, base = 10, precision = 3) {
  ...
  let result = Array.from({ length: n }, (_, i) => {
    return Math.pow(base, logX1 + (i / (n - 1)) * (logX2 - logX1)).toPrecision(
      precision
    );
  });
...
function cleanSeqData(dataMain, dataSeq) {
  ...
  if (
    ...
    dataSeq.forEach((row) => {
      ...
      if (
        !["undefined", undefined].includes(sampleGroup) &&
        !uniqueSampleGroups.includes(sampleGroup)
      ) {
    ...
    dataMain.forEach((row) => {
      ...
      for (let [colName, value] of Object.entries(row)) {
        if (seqNames.includes(colName)) {
          ...
          pointData["formattedAbundance"] = Number(
...
function makeRunSequencePlot(
  ...
  let [chemPlottingData, largestSeqIndex] = getPlottingDataForChem(
    plottingData,
    chemName
  );
  ...
  let yMin = d3.min(
    chemPlottingData.filter((d) => d.abundance > 0),
    (d) => d.abundance
  );
  let yMax = d3.max(
    chemPlottingData.filter((d) => d.abundance > 0),
  ...
  const yScale =
    yScaleType === "linear"
      ? d3
          .scaleLinear()
          .domain([0, yMax])
          .range([svgHeight - heightOffset, 50])
      : d3
          .scaleLog()
          .domain([yMin, yMax])
          .range([svgHeight - heightOffset, 50]);
  ...
  svg
    ...
    .on("mouseover", (event, d) => {
      ...
      d3.selectAll("tspan.tooltipTspan").style("font-weight", "bold");
  ...
  d3.selectAll(".tick > text").style("font-size", "13px");
  ...
  if (linesOfBestFit === false) {
    d3.selectAll("path.fit-line").style("visibility", "hidden");
  }
...
async function mainRunSequence(xlsxPath) {
  ...
  // add legend
  const svgL = makeSvgElement(
    170,
    200,
    "legendTooltip",
    "legendTooltip",
    legendTooltipGridContainer
  );
  ...
  buttonGridContainer
    ...
    .on("click", function () {
      ...
      d3.selectAll("path.fit-line").style("visibility", function () {
  ...
  function getGroupData() {
    ...
    groups.forEach((group) => {
      ...
      const groupDatum = [groupTitle, groupDesc, chemicalNames, chemicalModes];

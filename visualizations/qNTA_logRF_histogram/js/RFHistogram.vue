...
export default {
  ...
  methods: {
    createHistogram() {
       ...
       */
      async function readInterpretOutputXLSX() {
        const jsonData = XLSX.utils.sheet_to_json(
          workbook.Sheets[RF_VALUES_SHEET]
        );
        return jsonData;
      }
      ...
      async function histogramMain() {
        // read in data
        let data = await readInterpretOutputXLSX();
        ...
        const { posRFPercentiles, negRFPercentiles } =
          await readRFPercentileVals();
        ...
        let boxCoxRfValuesPos = rfValuesPos.map((d) =>
          boxCoxTransform(d, lambdaPos)
        );
        let boxCoxRfValuesNeg = rfValuesNeg.map((d) =>
          boxCoxTransform(d, lambdaNeg)
        );
        ...
        const parentGrid = d3
          ...
          .style("grid-template-rows", "1fr 4fr")
          .style(
            "grid-template-areas",
            `
      ...
      `
        ...
        const percentiles = Object.keys(posRFPercentiles[0]).filter(
        ...
        const rfRow = tableBody.append("tr").style("height", "28px");
        rfRow
          .append("td")
          .style("border", "1px solid black")
          .attr("id", "tina")
        ...
        const lambdaTooltip = parentGrid
          ...
          .style("padding", "10px")
          .style("box-shadow", "2px 2px 2px teal")
          .style("height", "50%");
        ...
        function showLambdaTooltip(event) {
          ...
          const lambdaValues =
            mode === "pos" ? lambdaValuesPos : lambdaValuesNeg;
        ...
        function showHelpTooltip(event) {
          ...
          this.$refs.helpTooltip.scrollTop = 0;
        }
        ...
        function makeHisto(dataArr, quantileValues = null) {
          ...
          svg
            ...
            .style("text-align", "center")
            .html(
              showingBoxCox ? "Box-Cox Transformed RF" : "Log<sub>10</sub>(RF)"
            );
          ...
          colors = colors.concat(
            generateColors("#009090", "#cc6600", 5).slice(1)
          );

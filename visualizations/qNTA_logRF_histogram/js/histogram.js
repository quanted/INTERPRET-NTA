...
async function histogramMain(inputXlsxPath) {
  ...
  var [ lambdaPos, lambdaValuesPos, llValuesPos ] = findOptimalLambda(rfValuesPos);
  lambdaPos = lambdaPos.toFixed(2);
  var [ lambdaNeg, lambdaValuesNeg, llValuesNeg ] = findOptimalLambda(rfValuesNeg);
  ...
  const parentGrid = d3.select("#histogram-container").append("div")
    .style("display", "grid")
  ...
  const quantileTable = parentGrid.append("table")
  ...
  headerRow.append("th").text("Q1").style("border", "1px solid black").style("padding", "5px");
  headerRow.append("th").text("Q2").style("border", "1px solid black").style("padding", "5px");
  headerRow.append("th").text("Q3").style("border", "1px solid black").style("padding", "5px");

  const body = quantileTable.append("tbody");

  const inputRow = body.append("tr").style("height", "28px");
  inputRow.append("td").style("border", "1px solid black")
    .html("<b>Quantile</b>")  
    .style("padding", "5px").style("text-align", "center");
  inputRow.append("td").style("border", "1px solid black")
    .append("div").style("display", "flex").style("padding", "3px")
    .append("input")
    .attr("class", "quantile-input")
    .attr("type", "number")
    .attr("step", "0.001")
    .attr("min", "0")
    .attr("max", "1")
    ...
    .style("width", "100%")
    .on("change", () => {
      if (d3.select("#quantile-0").property("value") > 1) {
        d3.select("#quantile-0").property("value", 1);
      } else if (d3.select("#quantile-0").property("value") < 0) {
        d3.select("#quantile-0").property("value", 0);
      }
    });
  inputRow.append("td").style("border", "1px solid black")
    .append("div").style("display", "flex").style("padding", "3px")
    ...
    .style("width", "100%")
    .on("change", () => {
      if (d3.select("#quantile-1").property("value") > 1) {
        d3.select("#quantile-1").property("value", 1);
      } else if (d3.select("#quantile-1").property("value") < 0) {
        d3.select("#quantile-1").property("value", 0);
      }
    });
  inputRow.append("td").style("border", "1px solid black")
    .append("div").style("display", "flex").style("padding", "3px")
    ...
    .style("width", "100%")
    .on("change", () => {
      if (d3.select("#quantile-2").property("value") > 1) {
        d3.select("#quantile-2").property("value", 1);
      } else if (d3.select("#quantile-2").property("value") < 0) {
        d3.select("#quantile-2").property("value", 0);
      }
    });

  const rfRow = body.append("tr");
  rfRow.append("td").style("border", "1px solid black")
    .attr("id", "tina")
    .html(("<b>Log<sub>10</sub>(RF)</b>"))
    .style("padding", "5.2px")
    .style("text-align", "center");
  rfRow.append("td").attr("id", "rf-0").style("border", "1px solid black").style("padding", "5px");
  rfRow.append("td").attr("id", "rf-1").style("border", "1px solid black").style("padding", "5px");
  rfRow.append("td").attr("id", "rf-2").style("border", "1px solid black").style("padding", "5px");

  // Create a box to display the lambda value
  var boxCoxToggled = false;
  const buttonPanel = parentGrid.append("div")
    .style("grid-area", "rightButtons");

  const lambdaBox = parentGrid.append("div")
    ...
    .on("mouseover", () => {
      d3.select("div.lambda-box").transition().duration(300)
    ...
    .on("click", function() {
      if (boxCoxToggled) {
        d3.select("#tina").html("<b>Log<sub>10</sub>(RF)</b>");
        d3.select(this).transition().duration(300)
          .style("background-color", "#eee");
      } else {
        d3.select("#tina").html("<b>Box-Cox Transformed RF</b>");
        d3.select(this).transition().duration(300)
  ...
  const lambdaTooltip = parentGrid.append("div")
    .style("z-index", 1000)
  ...
  lambdaTooltip.append("span")
    .attr("id", "lambda-tooltip-text")
    .style("display", "flex")
    .style("justify-content", "center")
    .style("font-size", "18px");

  const svgLLT = lambdaTooltip.append("svg")
  ...
  function showLambdaTooltip(event) {
    ...
    d3.select("#lambda-tooltip-text").html(`&#x03BB;${mode === "pos" ? "+" : "-"} = ${lambda}`);
    updateLambdaTooltip(lambda, lambdaValues, llValues);

    lambdaTooltip.transition()
      .duration(200)
      .style("opacity", 1);
  }
  ...
  const helpButton = buttonPanel.append("div")
    .style("grid-area", "rightButtons")
    ...
    .on("mouseover", () => {
      d3.select("div.help-button").transition().duration(300)
        .style("border-color", "black");
  ...
  const helpTooltip = parentGrid.append("div")
    .style("grid-area", "tooltips")
  ...
  function makeHisto(dataArr, quantileValues = null) {
    ...
    const svg = parentGrid.append("svg")
    ...
    svg.selectAll("rect")
      ...
      .enter().append("rect")
        ...
        .on("mouseover", function(event, d) {
          ...
          d3.selectAll("rect.histo-bin").transition().duration(200)
            .style("opacity", 0.7)
            .style("stroke-width", "1px");
          thisBin.transition().duration(200)
            .style("opacity", 1)
          ...
          tooltip.html(`<b>Count:</b> ${d.length}<br><b>Range:</b> ${d.x0.toFixed(2)} - ${d.x1.toFixed(2)}`);
        });

    d3.selectAll(".quantile-input").on("change", () => {
      const newQuantiles = [];
      for (let i = 0; i < 3; i++) {
        newQuantiles.push(parseFloat(d3.select(`#quantile-${i}`).property("value")));
      }
        d3.selectAll("line.quantile").remove();
        addQuantileLines(newQuantiles, x, height);
    });
      
    if (quantileValues === null) {
      quantileValues = [];
      for (let i = 0; i < 3; i++) {
        quantileValues.push(parseFloat(d3.select(`#quantile-${i}`).property("value")));
      } 
    } else {
      quantileValues = [0.025, 0.5, 0.975];
    }
    addQuantileLines(quantileValues, x, height);
  }
  ...
  function addQuantileLines(quantiles, x, height) {
    let dataArr;
    if (mode === "pos") {
      dataArr = showingBoxCox ? boxCoxRfValuesPos : logRfValuesPos;
    } else {
      dataArr = showingBoxCox ? boxCoxRfValuesNeg : logRfValuesNeg;
    }
    
    const sortedData = dataArr.sort(d3.ascending);
    const quantileValues = quantiles.map(q => d3.quantile(sortedData, q));

...
function sortData(data, category = null) {
  ...
  headers.forEach((key) => {
    if (
      (key.includes("NORM") || key.includes("norm")) &&
      !key.includes("TOTAL")
    ) {
      headers_norm.push(key);
    }
  });

  let itemToRemove = []; // List of rows containing no metadata, not to be included in the bar plots
  for (let i = 0; i < data.length; i++) {
    if (data[i]["STRUCTURE_TOTAL_NORM"] == 0) {
      itemToRemove.push(data[i]);
    }
    for (let j = 0; j < headers_norm.length; j++) {
      ...
      if (0 < data[i][currentHeader] && data[i][currentHeader] < 0.05) {
        data[i][currentHeader] = 0.05;
      }
    }
  }
  ...
  if (category != null) {
    newArray.sort((a, b) => b[category] - a[category]);
  }
  ...
  data.forEach((row) => {
    ...
    Object.entries(row).forEach(([key, value]) => {
      if (keysToKeep.includes(key)) {
        ...
        else if (keysToNum.includes(key)) {
          value = Number(Number(value).toPrecision(3));
        }
...
function truncateData(data, selectedFeature) {
  ...
  const result_truncated = data.filter(
    (feature) => feature["Feature ID"] == selectedFeature
  );
...
function getTop5Rows(arr, categoryField, valueField) {
  ...
  for (const category in groupedData) {
    const sortedData = groupedData[category].sort(
...
async function generatePlots(filePath) {
  ...
  const hasMS2 = Object.keys(fullData[0]).some((col) =>
    col.includes("Q-SCORE")
  );
  const hasHazard = Object.keys(fullData[0]).some((col) =>
    col.includes("Hazard Score")
  );
  ...
  if (hasMS2) {
    hasAmenability = Object.keys(fullData[0]).some((col) =>
      col.includes("AMENABILITY")
    );
  }

  function addInfoBox() {
    var settingsBorder = makeSvgElement(
      465,
      420,
      "settings-border",
      d3.select("#tripod-settings-container")
    );

    // Format and add elements to the info box based on presence of amenability and hazard data
    if (hasAmenability) {
      // if amenability data exists
      addMS2Legend();
      infoboxHeight = 300;
      infoboxTop = "482px";
      spanTop = "510px";
      ...
      settingsBorder
        ...
        .attr("y", 405);
      if (hasHazard) {
        // If hazard data and amenability data exists
        addHazardLegend();
        metadataTitleTop = 45;
        metadataTop = "-2px";
        buttonTop = "184px";
      ...
      //hazard data but no amenability data
      addHazardLegend();
      metadataTitleTop = 45;
      metadataTop = "-2px";
      infoboxHeight = 347;
      ...
      // no amenability and no hazard data
      infoboxHeight = 400;
      infoboxTop = "382px";
      spanTop = "460px";
      settingsHeight = 370;
    ...
    if (hasHazard) {
      document.getElementById("tripod-gradient-rect-hazard").style.position =
        "absolute";
      document.getElementById("tripod-gradient-rect-hazard").style.top =
        gradientTop;
    }
    ...
    document
      .getElementById("tripod-settings-container")
      .appendChild(structure_label_span);
    ...
    outlinksvg = makeSvgElement(
      30,
      30,
      "tripod-outlink-svg",
      d3.select("#tripod-outlinkDiv")
    );
    outlinksvgGroup = outlinksvg.append("g").attr("id", "tripod-outlink");
    outlinksvgGroup
      .append("rect")
      .attr("height", "500px")
      .attr("width", "500px")
    ...
    outlinksvgGroup
      ...
      .attr(
        "points",
        "400 464 48 464 48 104 240 104 240 72 16 72 16 496 432 496 432 272 400 272 400 464"
      );
    outlinksvgGroup
      .append("polygon")
      .attr("fill", "#BFBFBF")
      ...
      .attr(
        "points",
        "304 16 304 48 441.373 48 188.687 300.687 211.313 323.313 464 70.627 464 208 496 208 496 16 304 16"
      );
  ...
  var uniqueFeatureList = [
    ...new Set(fullData.map((item) => item["Feature ID"])),
  ];
  for (let i = 0; i < uniqueFeatureList.length; i++) {
    uniqueFeatureList[i] = Number(uniqueFeatureList[i]);
  }
  ...
  var abundance = Math.round(
    fullData[0]["Median blanksub mean feature abundance"]
  );
  ...
  function createTop5ToggleButton() {
    ...
    button.addEventListener("click", function () {
      ...
      if (showingTop5) {
        ...
        button.textContent =
          "Showing Top 5 metadata candidates. Click to show all";
        showingTop5 = true;
        makeLargeGrid();
        makeExportButton();
      ...
      if (clickedDTXCID) {
        try {
          fieldList.forEach((key) => {
            let IdToHighlight = document.getElementById(
              `ylabel-${clickedDTXCID}-${key}`
            );
  ...
  if (hasAmenability) {
    createAmenabilityToggleButtons();
  }
  ...
  function goToPosition(event, position) {
    ...
    if (position == "first") {
      selectedFeature = uniqueFeatureList[0];
    }
    // If position = last, shuttle to the last feature in the dataset
    else if (position == "last") {
      selectedFeature = uniqueFeatureList[uniqueFeatureListLength - 1];
    }
    // If position = forward, shuttle to the next feature in the dataset. If the current feature is the last feature in the dataset, do nothing.
      ...
      if (
        uniqueFeatureList.indexOf(selectedFeature) ==
        uniqueFeatureListLength - 1
      ) {
        return;
      }
    ...
    // If position = back, shuttle to the previous feature in the dataset. If the current feature is the first feature in the dataset, do nothing.
      ...
      if (uniqueFeatureList.indexOf(selectedFeature) == 0) {
        return;
      }
      let newIndex = uniqueFeatureList.indexOf(selectedFeature) - 1;
      selectedFeature = uniqueFeatureList[newIndex];
    ...
    // if position - input, shuttle to the feature specified in the input field by the user.
      ...
      if (uniqueFeatureList.includes(Number(input))) {
        selectedFeature = Number(input);
      } else {
        alert(
          `Feature ID '${input}' does not exist in the dataset. Please enter a valid Feature ID`
        );
        return;
      }
    }
  ...
  function makeArrows() {
    const arrowsvgleft = makeSvgElement(
      30,
      30,
      "tripod-arrow-left",
      d3.select("#tripod-chart-container")
    );
    ...
    arrowsvgleft
      ...
      .attr(
        "d",
        "M25 0H7a7 7 0 0 0-7 7v18a7 7 0 0 0 7 7h18a7 7 0 0 0 7-7V7a7 7 0 0 0-7-7zm5 25a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5h18a5 5 0 0 1 5 5z"
      );
    arrowsvgleft
      .append("path")
      .attr(
        "d",
        "m13.71 9.71-1.42-1.42-7 7a1 1 0 0 0 0 1.41l7 7 1.41-1.41L8.41 17H27v-2H8.41z"
      );
    document
      .getElementById("left-arrow")
      .addEventListener("click", function (event) {
        goToPosition(event, (position = "back"));
      });
    arrowsvgleft.attr(
      "onmouseover",
      "document.getElementById('left-arrow-rect').setAttribute('fill', '#3d4e634d')"
    );
    arrowsvgleft.attr(
      "onmouseout",
      "document.getElementById('left-arrow-rect').setAttribute('fill', '#DBE4F0')"
    );

    const arrowsvgright = makeSvgElement(
      30,
      30,
      "tripod-arrow-right",
      d3.select("#tripod-chart-container")
    ...
    arrowsvgright
      ...
      .attr(
        "d",
        "m19.71 8.29-1.42 1.42 5.3 5.29H5v2h18.59l-5.29 5.29 1.41 1.41 7-7a1 1 0 0 0 0-1.41z"
      );
    document
      .getElementById("right-arrow")
      .addEventListener("click", function (event) {
        goToPosition(event, (position = "forward"));
      });
    arrowsvgright.attr(
      "onmouseover",
      "document.getElementById('right-arrow-rect').setAttribute('fill', '#3d4e634d')"
    );
    arrowsvgright.attr(
      "onmouseout",
      "document.getElementById('right-arrow-rect').setAttribute('fill', '#DBE4F0')"
    );

    const arrowsvglast = makeSvgElement(
      30,
      30,
      "tripod-arrow-last",
      d3.select("#tripod-chart-container")
    ...
    document
      .getElementById("last-arrow")
      .addEventListener("click", function (event) {
        goToPosition(event, (position = "last"));
      });
    arrowsvglast.attr(
      "onmouseover",
      "document.getElementById('last-arrow-rect').setAttribute('fill', '#3d4e634d')"
    );
    arrowsvglast.attr(
      "onmouseout",
      "document.getElementById('last-arrow-rect').setAttribute('fill', '#DBE4F0')"
    );

    const arrowsvgfirst = makeSvgElement(
      30,
      30,
      "tripod-arrow-first",
      d3.select("#tripod-chart-container")
    ...
    document
      .getElementById("first-arrow")
      .addEventListener("click", function (event) {
        goToPosition(event, (position = "first"));
      });
    arrowsvgfirst.attr(
      "onmouseover",
      "document.getElementById('first-arrow-rect').setAttribute('fill', '#3d4e634d')"
    );
    arrowsvgfirst.attr(
      "onmouseout",
      "document.getElementById('first-arrow-rect').setAttribute('fill', '#DBE4F0')"
    );
    ...
    featureInput.setAttribute(
      "max",
      `${uniqueFeatureList[uniqueFeatureList.length - 1]}`
    );
    ...
    button.addEventListener("click", function (event) {
      goToPosition(event, (position = "input"));
    });
  }
  ...
  function addXaxis(xMax, pre_space) {
    ...
    const xAxisSvg = makeSvgElement(
      400,
      20,
      "tripod-xaxis",
      d3.select("#tripod-xaxis")
    );
    let x = d3.scaleLinear().domain([0, xMax]).range([0, 360]);
  ...
  function updateData(category) {
    ...
    Object.entries(subGroupData[0]).forEach(([key]) => {
      if (key.includes("NORM") && !key.includes("TOTAL")) {
        subGroupHeaders.push(key);
      }
    });

    //Check status of the Top-5 toggle button
    if (showingTop5) {
      subGroupData = sortData(
        unsorted_subGroupData,
        "STRUCTURE_TOTAL_NORM"
      ).slice(0, 5);
  ...
  function yAxisMeta(data) {
    ...
    yMeta = d3.scaleBand().domain(Ygroups).range([0, height]).padding([0.2]);
  ...
  function yAxisHazard(data) {
    height = subGroupData.length * 30;
    let Ygroups = data.map((d) => d.DTXCID);
    yHazard = d3.scaleBand().domain(Ygroups).range([0, height]).padding([0.2]);

    ylabelHazard = svgHazard
      .append("g")
      .attr("transform", `translate(135, 20)`)
      .call(d3.axisLeft(yHazard).tickSizeOuter(0))
      .attr("id", "tripod-ylabel-hazard");

    d3.select("#tripod-ylabel-hazard")
      .selectAll("text")
      .attr("id", function (d, i) {
        return "ylabel-" + Ygroups[i] + "-hazard";
      });

    // Extend the y-axis line if there are less than 23 candidates for the selected feature ID.
    if (Ygroups.length < 23) {
      ylabelHazard.select("path").attr("stroke", "white");
  ...
  function yAxisMS2(data) {
    height = subGroupData.length * 30;
    let Ygroups = data.map((d) => d.DTXCID);
    yMS2 = d3.scaleBand().domain(Ygroups).range([0, height]).padding([0.2]);
    ...
    d3.select("#tripod-ylabel-MS2")
      .selectAll("text")
      .attr("id", function (d, i) {
        return "ylabel-" + Ygroups[i] + "-MS2";
      });

    // Extend the y-axis line if there are less than 23 candidates for the selected feature ID.
    if (Ygroups.length < 23) {
      ylabelMS2.select("path").attr("stroke", "white");
  ...
  document
    .getElementById("tripod-outlink")
    .addEventListener("click", goToHyperlink);
  ...
  var ylabelClick = function (event) {
    var DTXCIDname = d3
      .select(this)
      ["_groups"][0][0].querySelector("text").innerHTML;
    structure_label.nodeValue = DTXCIDname;

    //Get the other features that this DTXCID is a candidate for
    const featureArray = fullData
      .filter((d) => d.DTXCID === DTXCIDname)
      .map((d) => d["Feature ID"]);
    ...
    outlinkDiv.addEventListener("mouseover", function () {
      imageDiv.style.borderWidth = "3px";
    });
    outlinkDiv.addEventListener("mouseout", function () {
      imageDiv.style.borderWidth = "1px";
    });
    imageDiv.addEventListener("mouseover", function () {
      (imageDiv.style.borderWidth = "3px"), (imageDiv.style.cursor = "pointer");
    });
    ...
    if (previousClickedDTXCID == clickedDTXCID) {
      fieldList.forEach((key) => {
        let IdToHighlight = document.getElementById(
          `ylabel-${DTXCIDname}-${key}`
        );
        IdToHighlight.setAttribute("fill", "red");
        IdToHighlight.style.fontWeight = "bold";
        if (
          !isElementVisibleInScrollContainer(
            document.getElementById(`ylabel-${DTXCIDname}-${key}`),
            document.getElementById(`tripod-chart-${key}`)
          )
        ) {
          document
            .getElementById(`ylabel-${DTXCIDname}-${key}`)
      ...
      fieldList.forEach((key) => {
        ...
        if (previousClickedDTXCID) {
          try {
            let IdToHighlight2 = document.getElementById(
              `ylabel-${previousClickedDTXCID}-${key}`
            );
  ...
  var barClickMeta = function () {
    var DTXCIDname = document.getElementById(
      `ylabel-${
        d3.select(this)._groups[0][0]["__data__"]["data"]["DTXCID"]
      }-meta`
    ).innerHTML;
    structure_label.nodeValue = DTXCIDname;
  ...
  var barClickMS2Hazard = function () {
    if (hasMS2) {
      var DTXCIDname = document.getElementById(
        `ylabel-${d3.select(this)._groups[0][0]["__data__"]["DTXCID"]}-MS2`
      ).innerHTML;
    }
    if (hasHazard) {
      var DTXCIDname = document.getElementById(
        `ylabel-${d3.select(this)._groups[0][0]["__data__"]["DTXCID"]}-hazard`
      ).innerHTML;
    }
    structure_label.nodeValue = DTXCIDname;
    ...
    if (this.className["baseVal"] == "hazard-bar") {
      window.open(
        `https://hazard-dev.sciencedataexperts.com/#/hazard/report/${DTXCIDname}`
      );
      ...
      const exp_spec =
        d3.select(this)._groups[0][0]["__data__"]["experimental_spectrum"];

      window.open(
        `mirror_plots.html?dtxcid=${DTXCIDname}&feature=${selectedFeature}&CFMID_10=${CFMID_10}&CFMID_20=${CFMID_20}&CFMID_40=${CFMID_40}&exp_spec=${exp_spec}`,
        "PopupWindow",
  ...
  var mousemoveYlabel = function (event) {
    structureToolTip.style("display", "block");
  ...
  var mouseoverYlabel = function (d) {
    ...
    function getImageY() {
      ...
      imageY.style =
        "width:90px;height:90px;padding-top:2px;padding-bottom:2px;";
  ...
  var mouseleaveYlabel = function () {
    ...
    tooltipYlabel.style("display", "none");
  ...
  function makeLegend() {
    const legendData = d3
      .stack()
      .keys(keysToInclude)(subGroupData)
      .sort((a, b) => a.key.localeCompare(b.key));
    // Add the legend
    const legendsvg = makeSvgElement(
      450,
      200,
      "metadata-legendbox",
      d3.select("#metadata-legend")
    );
    ...
    var legendClick = function (event, d, i) {
      ...
      if (currentDecoration === "line-through") {
        //If the text is already lined out....
        d3.select(this).style("text-decoration", "none"); //Remove the line from the text
        document.getElementById("metadata-square" + currentIndex).style.fill =
          stackColors[currentIndex]; //white-out the color square
        showKeys.push(allKeys[currentIndex]); //Add the selected metadata to the showKeys list
        removedKeys = removedKeys.filter(
          (item) => item !== allKeys[currentIndex]
        ); //removed the key from the removed keys list
        ...
        document.getElementById("metadata-square" + currentIndex).style.fill =
          "white"; //white-out the color square
  ...
  function makeTitleStatic() {
    const titlesvg = makeSvgElement(
      500,
      20,
      "tripod-title",
      d3.select("#tripod-title-static")
    );
    titlesvg
      .append("rect")
      .attr("width", 1610)
  ...
  function loadData(data) {
    ...
    var tooltipBarMeta = d3
      .select("#tripod-chart-meta")
      .append("div")
      .style("display", "none")
      .attr("class", "tooltip")
      .style("background-color", "white")
    ...
    if (hasMS2) {
      var tooltipBarMS2 = d3
        .select("#tripod-chart-MS2")
        .append("div")
        .style("display", "none")
        .attr("class", "tooltip")
        .style("background-color", "white")
    ...
    if (hasHazard) {
      var tooltipBarHazard = d3
        .select("#tripod-chart-hazard")
        .append("div")
        .style("display", "none")
        .attr("class", "tooltip")
        .style("background-color", "white")
    ...
    var mouseoverBarMeta = function (d) {
      ...
      if (categoryName.includes("fact")) {
        ...
        var categoryValue =
          d.srcElement.__data__.data[catName + "_COUNT_COLLAPSED"];
      ...
      tooltipBarMeta.html(labelName + ": " + categoryValue).style("opacity", 1);
    ...
    var mouseleaveBarMeta = function () {
      tooltipBarMeta.style("display", "none");
    ...
    var mouseoverBarMS2 = function (d) {
      if (!hasAmenability) {
        ...
        tooltipBarMS2.html("MS2 Score: " + MS2Score).style("opacity", 1);
      } else {
        var MS2Score = d3.select(this)._groups[0][0]["__data__"]["Q-SCORE"];
        var amenabilityScore =
          d3.select(this)._groups[0][0]["__data__"][amenabilityMode];
        if (amenabilityScore == -9999) {
          amenabilityScore = "No Data";
        }
        tooltipBarMS2
          .html(
            "MS2 Score: " +
      ...
      fieldList.forEach((key) => {
        let IdToHighlight = document.getElementById(
          `ylabel-${d3.select(this)._groups[0][0]["__data__"]["DTXCID"]}-${key}`
        );
        IdToHighlight.setAttribute("fill", "#FF13F0");
        IdToHighlight.style.fontWeight = "bold";
      });
    ...
    var mouseleaveBarMS2Hazard = function () {
      if (hasMS2) {
        tooltipBarMS2.style("display", "none");
      }
      if (hasHazard) {
      ...
      if (
        clickedDTXCID != d3.select(this)._groups[0][0]["__data__"]["DTXCID"]
      ) {
        fieldList.forEach((key) => {
          let IdToHighlight = document.getElementById(
            `ylabel-${
              d3.select(this)._groups[0][0]["__data__"]["DTXCID"]
            }-${key}`
          );
          IdToHighlight.setAttribute("fill", "black");
          IdToHighlight.style.fontWeight = "normal";
    ...
    var mouseoverBarHazard = function (d) {
      ...
      var HCompScore =
        d3.select(this)._groups[0][0]["__data__"]["Hazard Completeness Score"];
      tooltipBarHazard
        .html(
          "Hazard Score: " +
            HScore +
    ...
    showBarsHazard = function (data) {
      svgHazard.append("g");
      hazard_bars = svgHazard
        .selectAll(".bar")
        .data(data)
        .enter()
        .append("rect")
        .attr("fill", "red")
        .attr("opacity", (d) => d["Hazard Completeness Score"])
        .attr("transform", `translate(137, 20)`)
    ...
    if (metaInput.checked) {
      svgMeta = makeSvgElement(
        width,
        height + 20,
        "tripod-vis",
        d3.select("#tripod-chart-meta")
      );
    ...
    if (hasHazard && hazardInput.checked) {
      svgHazard = makeSvgElement(
        width,
        height + 20,
        "tripod-vis",
        d3.select("#tripod-chart-hazard")
      );
    ...
    if (hasMS2 && MS2Input.checked) {
      svgMS2 = makeSvgElement(
        width,
        height + 20,
        "tripod-vis",
        d3.select("#tripod-chart-MS2")
      );
    ...
    function makeTitle() {
      ...
      titlesvg
        ...
        .style("font-size", "20px")
        .text(
          `Mass: ${mass}    RT: ${RT}          Median Abundance: ${abundance}    Occurrence: ${occ_percentage}%`
        );
    }
  ...
  function createInfoTooltip() {
    ...
    function infoCircleMouseover() {
      ...
      tooltipInfo
        .html(
          "Each metadata bar segment is normalized to a value between 0 and 1 based on the highest value among all DTXCIDs for a given feature, with a minimum bar width of 0.05."
        )
        .style("opacity", 1);
    }
  ...
  function makeLargeGrid() {
    ...
    if (showingTop5) {
      ...
      gridData = cleanData(fullData, keysToKeep);
    }
    ...
    var columnDefs = [
          ...
          },
          {
            ...
            cellRenderer: (params) => {
              try {
                ...
                image.style =
                  "width:80px;height:80px;padding-top:2px;padding-bottom:2px;";
                image.alt = `Structure image for ${params.data.DTXCID}`;
                return image;
              } catch (error) {
                var p = document.createElement("div");
                p.style =
                  "width:70px;height:70px;padding-top:2px;padding-bottom:2px;text-align: center; line-height: 70px;";
        ...
        children: [
          {
            ...
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Q-SCORE"] ?? "N/A";
            },
          },
          {
          ...
          {
            columnGroupShow: "open",
            headerName: "(pos) Amenability Score",
            field: "POSITIVE_MODE_AMENABILITY_PREDICTION",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
            ...
            valueGetter: (params) => {
              ...
              if (value === -9999) {
                ...
                return (
                  params.data?.["POSITIVE_MODE_AMENABILITY_PREDICTION"] ?? "N/A"
                );
              }
            },
          ...
          {
            columnGroupShow: "open",
            headerName: "(neg) Amenability Score",
            field: "NEGATIVE_MODE_AMENABILITY_PREDICTION",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
            ...
            valueGetter: (params) => {
              ...
              if (value === -9999) {
                ...
                return (
                  params.data?.["NEGATIVE_MODE_AMENABILITY_PREDICTION"] ?? "N/A"
                );
              }
            },
            comparator: (valueA, valueB, isDescending) => {
      ...
      },
      {
        ...
        children: [
          {
            columnGroupShow: "closed",
            headerName: "Metadata Score",
            field: "STRUCTURE_TOTAL_NORM",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 200,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "AMOS Fact Sheets",
            field: "Structure_AMOS fact sheets count",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "AMOS Methods",
            field: "Structure_AMOS methods count",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 130,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "AMOS Spectra",
            field: "Structure_AMOS spectra count",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 130,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "PubMed Articles",
            field: "PUBMED_COUNT_COLLAPSED",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "PubChem Articles",
            field: "LITERATURE_COUNT_COLLAPSED",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "PubChem Patents",
            field: "PATENT_COUNT_COLLAPSED",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "PubChem Sources",
            field: "SOURCE_COUNT_COLLAPSED",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 150,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Dashboard Water Lists",
            field: "Structure_Presence in water lists count",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 175,
            sortingOrder: ["desc", "asc", null],
        ...
        children: [
          {
            ...
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Hazard Score"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "closed",
            headerName: "Hazard Completeness Score",
            field: "Hazard Completeness Score",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 210,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Hazard Completeness Score"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
          ...
          {
            columnGroupShow: "open",
            headerName: "Acute Aquatic Toxicity Authority",
            field: "Acute Aquatic Toxicity_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Acute Aquatic Toxicity_authority_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Acute Aquatic Toxicity Score",
            field: "Acute Aquatic Toxicity_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Acute Aquatic Toxicity_score_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Dermal Authority",
            field: "Acute Mammalian Toxicity Dermal_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Dermal Score",
            field: "Acute Mammalian Toxicity Dermal_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Acute Mammalian Toxicity Dermal_score_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Inhalation Authority",
            field: "Acute Mammalian Toxicity Inhalation_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Inhalation Score",
            field: "Acute Mammalian Toxicity Inhalation_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Oral Authority",
            field: "Acute Mammalian Toxicity Oral_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Acute Mammalian Toxicity Oral Score",
            field: "Acute Mammalian Toxicity Oral_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Acute Mammalian Toxicity Oral_score_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Bioaccumulation Authority",
            field: "Bioaccumulation_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Bioaccumulation_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Bioaccumulation Score",
            field: "Bioaccumulation_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Bioaccumulation_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Carcinogenicity Authority",
            field: "Carcinogenicity_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Carcinogenicity_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Carcinogenicity Score",
            field: "Carcinogenicity_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Carcinogenicity_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Chronic Aquatic Toxicity Authority",
            field: "Chronic Aquatic Toxicity_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Chronic Aquatic Toxicity_authority_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Chronic Aquatic Toxicity Score",
            field: "Chronic Aquatic Toxicity_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Chronic Aquatic Toxicity_score_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Developmental Authority",
            field: "Developmental_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Developmental_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Developmental Score",
            field: "Developmental_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Developmental_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Endocrine Disruption Authority",
            field: "Endocrine Disruption_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Endocrine Disruption_authority_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Endocrine Disruption Score",
            field: "Endocrine Disruption_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Endocrine Disruption_score_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Exposure Authority",
            field: "Exposure_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Exposure_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Exposure Score",
            field: "Exposure_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Exposure_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Eye Irritation Authority",
            field: "Eye Irritation_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Eye Irritation_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Eye Irritation Score",
            field: "Eye Irritation_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Eye Irritation_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Genotoxicity Mutagenicity Authority",
            field: "Genotoxicity Mutagenicity_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Genotoxicity Mutagenicity_authority_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Genotoxicity Mutagenicity Score",
            field: "Genotoxicity Mutagenicity_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Genotoxicity Mutagenicity_score_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Neurotoxicity Repeat Exposure Authority",
            field: "Neurotoxicity Repeat Exposure_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Neurotoxicity Repeat Exposure Score",
            field: "Neurotoxicity Repeat Exposure_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Neurotoxicity Repeat Exposure_score_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Neurotoxicity Single Exposure Authority",
            field: "Neurotoxicity Single Exposure_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Neurotoxicity Single Exposure Score",
            field: "Neurotoxicity Single Exposure_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Neurotoxicity Single Exposure_score_mapped"] ??
                "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Persistence Authority",
            field: "Persistence_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Persistence_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Persistence Score",
            field: "Persistence_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Persistence_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Reproductive Authority",
            field: "Reproductive_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Reproductive_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Reproductive Score",
            field: "Reproductive_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Reproductive_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Skin Irritation Authority",
            field: "Skin Irritation_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Skin Irritation_authority_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Skin Irritation Toxicity Score",
            field: "Skin Irritation_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Skin Irritation_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Skin Sensitization Authority",
            field: "Skin Sensitization_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return (
                params.data?.["Skin Sensitization_authority_mapped"] ?? "N/A"
              );
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Skin Sensitization Score",
            field: "Skin Sensitization_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
            valueGetter: (params) => {
              return params.data?.["Skin Sensitization_score_mapped"] ?? "N/A";
            },
          },
          {
            columnGroupShow: "open",
            headerName: "Systemic Toxicity Repeat Exposure Authority",
            field: "Systemic Toxicity Repeat Exposure_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Systemic Toxicity Repeat Exposure Score",
            field: "Systemic Toxicity Repeat Exposure_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Systemic Toxicity Single Exposure Authority",
            field: "Systemic Toxicity Single Exposure_authority_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
          ...
          {
            columnGroupShow: "open",
            headerName: "Systemic Toxicity Single Exposurey Score",
            field: "Systemic Toxicity Single Exposure_score_mapped",
            floatingFilter: true,
            filter: "agNumberColumnFilter",
            width: 100,
            sortingOrder: ["desc", "asc", null],
    ...
    function onGridFilter() {
      ...
      if (hasMS2) {
        originallyCheckedMS2 = MS2Input.checked;
      }
      if (hasHazard) {
        originallyCheckedHazard = hazardInput.checked;
      }
  ...
  function makeExportButton() {
    const exportButton = makeSvgElement(
      30,
      30,
      "tripod-export-button",
      d3.select("#tripod-grid")
    );
    ...
    exportButton
      ...
      .attr(
        "d",
        "M7.25 17C7.25 16.5858 7.58579 16.25 8 16.25H16C16.4142 16.25 16.75 16.5858 16.75 17C16.75 17.4142 16.4142 17.75 16 17.75H8C7.58579 17.75 7.25 17.4142 7.25 17Z"
      )
      .attr("transform", "translate(-5.5, -6) scale(1.8) ");

    exportButton.attr(
      "onmouseover",
      "document.getElementById('export-button-rect').setAttribute('fill', '#3d4e634d')"
    );
    exportButton.attr(
      "onmouseout",
      "document.getElementById('export-button-rect').setAttribute('fill', '#DBE4F0')"
    );
    document
      .getElementById("tripod-export-button")
      .addEventListener("click", function (event) {
        ...
        window.gridAPI.exportDataAsCsv({
          fileName: "candidate_ranking_grid_data.csv",
